	...
	function fallback:GetNearestNodeOrNil(pos)
		...
		for id, node in pairs(self.NodeById) do
			...
			if node.HasArea then
				...
				nodePos = vec(math_Clamp(pos.x, params.AreaXMin, params.AreaXMax), math_Clamp(pos.y, params.AreaYMin, params.AreaYMax), nodePos.z)
			end
			local dist = pos:DistToSqr(nodePos)
