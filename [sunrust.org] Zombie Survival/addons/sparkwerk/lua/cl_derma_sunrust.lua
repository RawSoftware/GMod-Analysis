...
SKIN.PrintName		= "Sunrust Derma Skin"
SKIN.Author		= "Extreme56"
SKIN.DermaVersion	= 1
SKIN.GwenTexture	= Material("gwenskin/sunrust.png")
...
SKIN.text_dark					= Color( 255, 82, 0, 255 )
...
SKIN.panel_transback			= Color( 128, 128, 128, 50 )
...
function SKIN:PaintPanel( panel, w, h )
	if panel.Label and not panel.FixedLabelColor then
		if panel.Label:GetTextColor() == Color(0, 0, 0) then
			panel.Label:SetTextColor(self.Colours.Tree.Normal)
		end
		panel.FixedLabelColor = true
	end
...
function SKIN:PaintMenu( panel, w, h )
	...
	if ( panel:GetDrawColumn() ) then
		...
		self.tex.MenuBG( 0, 0, w, h, self.control_color_dark )
	end
...
function SKIN:PaintMenuSpacer( panel, w, h )
	...
	surface.SetDrawColor( Color( 178, 58, 0, 100 ) )
	surface.DrawRect( 0, 0, w, h )

end

local menu_def_color = Color( 10, 10, 10 )

--[[---------------------------------------------------------
	MenuOption
-----------------------------------------------------------]]
function SKIN:PaintMenuOption( panel, w, h )
	if not panel.FixTextCol and panel:GetTextColor() == menu_def_color then
		panel.FixTextCol = true
		panel:SetTextColor(Color(255,255,255))
	end

	if ( panel.m_bBackground && (panel.Hovered || panel.Highlight) ) then
		self.tex.MenuBG_Hover( 0, 0, w, h, Color(200, 200, 200, 255) )
	end
...
function SKIN:PaintListView( panel, w, h )
	...
	self.tex.Input.ListBox.Background( 0, 0, w, h, self.control_color_dark )
...
derma.DefineSkin("sunrust", "Sunrust Theme", SKIN)
