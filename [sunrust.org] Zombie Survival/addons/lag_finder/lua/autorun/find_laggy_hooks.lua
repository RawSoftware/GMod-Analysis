...
local IntroMessage = "\n\nHere are the %u laggiest hooks found during the last %u frames:\n"..
	( SERVER and game.IsDedicated() and " Identifier ------------------------------- Impact  Execs  Mean t /f  MaxÂ time \n" or--79
	" Identifier ------------------------------------------------------------ Impact  Executions  Mean t /f  MaxÂ time \n" )--113
local FormatTimings
do
		...
		for k=1,TopCount do
			...
			if addon_hooks_conflict_finder and addon_hooks_conflict_finder.LocateLuaFile then
				...
				elseif #locations == 1 then
					foundstr = "found in Workshop addon Â«Â "..locations[1].."Â Â»"
				else
					foundstr = "found in Workshop addons Â«Â "..string.Implode( "Â Â», Â«Â ", locations ).."Â Â»"
				end
			else
